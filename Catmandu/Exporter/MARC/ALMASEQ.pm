...
Catmandu::Exporter::MARC::ALMASEQ - Exporter for MARC records to Ex Libris' Aleph sequential - adapted for Alma system numbers

=head1 SYNOPSIS

    # From the command line
    $ catmandu convert MARC to MARC --type ALMASEQ < /foo/data.mrc

    # From Perl
    use Catmandu;

    my $importer = Catmandu->importer('MARC', file => "/foo/bar.mrc" , type => 'ISO');
    my $exporter = Catmandu->exporter('MARC', file => "marc.txt", type => 'ALMASEQ' );
...
=cut
package Catmandu::Exporter::MARC::ALMASEQ;
use Catmandu::Sane;
