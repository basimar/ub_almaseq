...
Catmandu::Importer::MARC::ALMASEQ - Package that imports Ex Libris' Alma sequential MARC records

=head1 SYNOPSIS

    # From the command line
    $ catmandu convert MARC --type ALMASEQ --fix "marc_map('245a','title')" < /foo/usm01.txt

    # From perl
    use Catmandu;

    # import records from file
    my $importer = Catmandu->importer('MARC',file => '/foo/usm01.txt' , type => 'ALMASEQ');
...
=cut
package Catmandu::Importer::MARC::ALMASEQ;
use Catmandu::Sane;
use Moo;
...
sub generator {
    ...
    sub {
        ...
        while(<$fh>) {
           chop;
           next unless (length $_ >= 28);

           my ($sysid,$s1,$tag,$ind1,$ind2,$s2,$char,$s3,$data) = unpack("A19A1A3A1A1A1A1A1U0A*",$_);
           $sysid =~ s/ *$//g;
